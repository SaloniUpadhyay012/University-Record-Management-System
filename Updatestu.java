...
public class Updatestu extends JFrame implements ActionListener {
    ...
    Updatestu() {
        ...
        JLabel lablrn = new JLabel("Roll No.");
        ...
        crn.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent ie) {
                try {
                    conn c = new conn();
                    String query = "select * from student where rollno='" + crn.getSelectedItem() + "'";
                    ResultSet rs = c.s.executeQuery(query);
                    if (rs.next()) {
                        lblname.setText(rs.getString("name"));
                        lblfname.setText(rs.getString("fathername"));
                        labelrn.setText(rs.getString("rollno"));
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == submit) {
            ...
            if (name.isEmpty() || fatherName.isEmpty() || rollNo.isEmpty() || dob.isEmpty() ||
                ...
                conn c = new conn();
                try {
                    String query = "UPDATE student SET name = '" + name + "', fathername = '" + fatherName + "', dob = '" + dob + "', "
                            + "address = '" + address + "', phone = '" + phone + "', email = '" + email + "', course = '" + course + "', branch = '" + branch + "' "
                            + "WHERE rollno = '" + rollNo + "'";
