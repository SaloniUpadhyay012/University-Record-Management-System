...
public class sinfo extends JFrame implements ActionListener {
    ...
    sinfo() {
        ...
        add(heading);heading.setFont(new Font("serif", Font.BOLD, 30));
    ...
    if (ae.getSource() == submit) {
        ...
        if (name.isEmpty() || fatherName.isEmpty() || rollNo.isEmpty() || dob.isEmpty() ||
            ...
            conn c = new conn();
            try {
                String query = "INSERT INTO student (name, fatherName, rollNo, dob, address, phone, email, X, XII, aadhar, course, branch) " +
                               "VALUES ('" + name + "', '" + fatherName + "', '" + rollNo + "', '" + dob + "', '" + address + "', '" + phone + "', '" + email + "', '" + X + "', '" + XII + "', '" + aadhar + "', '" + course + "', '" + branch + "')";
